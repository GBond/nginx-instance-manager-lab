...
Navigate to the UDF Instances and look for ``NGINX Plus - 2``.  Select 
the ``ACCESS`` dropdown and open the ``SAMPLE WEB PAGE`` selection.

.. image:: ./UDF-nginx5-sample.png

A default web page that says ``Welcome to CentOS`` should appear.
...
Click under modules, then instances and click under actions edit config on Ubuntu with NGINX type 1.21.5
...
Click the ``Save as`` button at the top then save.
...
For this step, we are going to use the editor and the ssh session to ``NGINX Plus - 2``.
...
Navigate to the NGINX Plus - 2 instance with SSH and see if the file is present.
...
Now select ``nginx.conf`` in the include dropdown menu for ``NGINX Plus - 2`` and remove the line we just added for including test.conf.
...
Navigate to the NGINX Plus - 2 instance with ssh and see if the files is still present.
...
Let's use a simple example and focus on the ``nginx.conf`` file and ``NGINX Plus - 2
``. 
...
SSH to ``NGINX Plus - 2`` and run the following command to see what changed.
